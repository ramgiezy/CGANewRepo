...
std::vector<glm::vec3> lamp1Position = { glm::vec3(-7.03, 0, -19.14), glm::vec3(
		24.41, 0, -34.57), glm::vec3(-10.15, 0, -54.10), glm::vec3(17.96f, 0, -51.08f) };
std::vector<float> lamp1Orientation = { -17.0, -82.67, 23.70, -40 };
std::vector<glm::vec3> lamp2Position = { glm::vec3(-36.52, 0, -23.24),
		glm::vec3(-52.73, 0, -3.90), glm::vec3(34.18f, 0.0f, 8.98f) };
std::vector<float> lamp2Orientation = {21.37 + 90, -65.0 + 90, -130.2};
...
void applicationLoop() {
	...
	while (psi) {
		...
		for (int i = 0; i < lamp2Position.size(); i++) {
			...
			matrixAdjustLamp = glm::translate(matrixAdjustLamp, glm::vec3(0.785f, 4.375f, 0.0f));
			glm::vec3 lampPosition = glm::vec3(matrixAdjustLamp[3]);
		...
		for (int i = 0; i < lamp2Position.size(); i++) {
			...
			modelLamp2.setScale(glm::vec3(1.0f, 1.0f, 1.0f));
			modelLamp2.setOrientation(glm::vec3(0, lamp2Orientation[i], 0));
			modelLamp2.render();

			modelLamp2Post.setPosition(lamp2Position[i]);
			modelLamp2Post.setScale(glm::vec3(1.0f, 1.0f, 1.0f));
