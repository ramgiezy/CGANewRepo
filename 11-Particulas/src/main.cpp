...
// Definicion de variables para el sist de particulas
...
void inicializacionParticulasFuente() {
	...
	// LLenar las velocidades iniciales con aleatorias
	glm::vec3 v(0.0);
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	// Definiendo la textura de particulas
...
bool processInput(bool continueApplication) {
	...
	if (modelSelected == 2 && glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS){
		modelMatrixMayow = glm::rotate(modelMatrixMayow, glm::radians(1.5f), glm::vec3(0, 1, 0));
		animationIndex = 0;
	}else if (modelSelected == 2 && glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS){
		modelMatrixMayow = glm::rotate(modelMatrixMayow, glm::radians(-1.5f), glm::vec3(0, 1, 0));
		animationIndex = 0;
	}if (modelSelected == 2 && glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS){
		modelMatrixMayow = glm::translate(modelMatrixMayow, glm::vec3(0, 0, 0.22));
		animationIndex = 0;
	}else if (modelSelected == 2 && glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS){
		modelMatrixMayow = glm::translate(modelMatrixMayow, glm::vec3(0, 0, -0.22));
		animationIndex = 0;
...
void applicationLoop() {
	...
	while (psi) {
		...
		shaderTerrain.setMatrix4("view", 1, false,
				glm::value_ptr(view));
		//settea matriz de vista y projeccion al shader de particulas de fuente
		...
		for(std::map<float, std::pair<std::string, glm::vec3> >::reverse_iterator it = blendingSorted.rbegin(); it != blendingSorted.rend(); it++){
			if(it->second.first.compare("aircraft") == 0){
				...
				if (currTimeParticulasFuente - lastTimeParticulasFuente > 15.0)
				...
				shaderParticulasFountain.setFloat("ParticleLifetime", 8.5f);
