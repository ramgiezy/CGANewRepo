...
void applicationLoop() {
	...
	while (psi) {
		...
		//Matriz de proyeccion para la luz que genera las sombras
		...
		// El primer paso es renderizar el buffer de profundidad
		...
		// Solo para ver lo que se dibuja desde la luz
		// Reset viewport
		/*glViewport(0, 0, screenWidth, screenHeight);
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
		...
		//Paso 2 renderizar los objetos de la escena normal
		glViewport(0, 0, screenWidth, screenHeight);
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
		prepareScene();
